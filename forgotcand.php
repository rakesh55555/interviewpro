...
<center><p><b>Here enter your email to recover your password</b></p>
  <div class="container container-fluid">
    ...
    <div class="panel panel-info">
        <div class="panel-heading"><h3>Password recovery for Candidate</h3></div>
     ...
     <form class="form-horizontal"  method="post">
          <fieldset>
              <div class="form-group">
                <label for="inputEmail" id="forg" class="col-lg-3 control-label"><span class="glyphicon glyphicon-euro"></span>&nbsp;&nbsp;Email</label>
                  <div class="col-lg-8">
                    <input class="form-control" id="forg" placeholder="Email" type="email" name="Email1">
                  </div>
              </div>

              <button type="submit" id="forg" name="btnforgot1" class="btn btn-info">Submit</button>
             <br><br><a href="candlogin.php"><font size='5'>Log In</font></a>
        </fieldset>
         <?php
           require("connect.php");
        if(isset($_POST['btnforgot1']))
           {
             ...
             $res = mysql_query($sql); 
              if($row= mysql_fetch_assoc($res))
              {
                  ...
                  $id=$row['candidate_acc_id'];
                
                  $nm=$row['name'];
                  $pw=$row['password'];
                  $qn=$row['qnans'];
                  echo "<b>Hii Mr.  ".$nm."  Enter your 1st School Name</b><input type='text' name='qns' required ><button type='submit' value='".$id."' class='btn btn-info' name='btnschooll1'>Submit</button>";
              } 
              else{
        ...
        if(isset($_POST['btnschooll1']))
           {
            echo "<style>#forg{display:none;}</style>";
            $q =$_POST['btnschooll1'];
            $v= $_POST['qns'];
            $sql1= "SELECT * FROM candidate_account WHERE candidate_acc_id='".$q."'";
              ...
              if(isset($_POST['pwup']))
           {
            $p=$_POST['nwpw'];
            $q1=$_POST['pwup'];
             $sql3=" UPDATE  `candidate_account` SET `password`= '$p'  WHERE  `candidate_acc_id`=".$q1;
            if(mysql_query($sql3))
                {
